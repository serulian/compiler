...
func newSimpleDirectionalIterator(layer *GraphLayer, startingName string, prefixedPredicate string, direction int) NodeIterator {
	startingValue := layer.cayleyStore.ValueOf(startingName)
...
type simpleDirectionalIterator struct {
	...
	prefixedPredicate string             // The predicate to check, already prefixed for the layer.
...
func (sdi simpleDirectionalIterator) Node() GraphNode {
	quad := sdi.layer.cayleyStore.Quad(sdi.quadIterator.Result())
	return sdi.layer.GetNode(quad.Get(sdi.direction))
}

func (sdi simpleDirectionalIterator) Values() map[string]string {
	// Note: No values in simple iterators.
	return map[string]string{}
}

func (sdi simpleDirectionalIterator) TaggedValue(predicate string, example TaggedValue) interface{} {
	// Note: No values in simple iterators.
	return nil
}
