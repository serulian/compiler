...
// TryGetContainingTypeOrMember returns the type or member that contains the given node, if any.
func (f *SourceStructureFinder) TryGetContainingTypeOrMember(node compilergraph.GraphNode) (SRGTypeOrMember, bool) {
	typeOrMemberNode, found := f.tryGetContainingNode(node, TYPE_OR_MEMBER_KINDS...)
	if !found {
		return SRGTypeOrMember{}, false
	}

	return SRGTypeOrMember{typeOrMemberNode, f.srg}, true
}
