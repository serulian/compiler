...
func (sb *scopeBuilder) scopeFullLambaExpression(node compilergraph.GraphNode) proto.ScopeInfo {
	...
	if hasReturnType {
		resolvedReturnType, rerr := sb.sg.tdg.BuildTypeRef(sb.sg.srg.GetTypeRef(returnTypeNode))
		if rerr != nil {
			panic(rerr)
		}
	...
	for pit.Next() {
		...
		parameterType, perr := sb.sg.tdg.BuildTypeRef(sb.sg.srg.GetTypeRef(parameterTypeNode))
		if perr != nil {
			panic(perr)
		}
