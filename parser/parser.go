...
func ParseExpression(builder NodeBuilder, source compilercommon.InputSource, startIndex int, input string) (AstNode, bool) {
	noopHandler := func(kind string, importPath string, packageImportType packageloader.PackageImportType, importSource compilercommon.InputSource, runePosition uint64) string {
		return ""
	}
...
func (p *sourceParser) reportImport(value string, kind string) (string, error) {
	...
	return p.importReporter(kind, importPath, packageImportType, p.source, uint64(p.currentToken.position)), nil
}
