...
type isWhitespaceTokenChecker func(kind generic.T) bool
type lexSourceImpl func(l *lexer) stateFn

// buildlex creates a new scanner for the input string.
func buildlex(source compilercommon.InputSource, input string, impl lexSourceImpl, whitespace isWhitespaceTokenChecker, errorTokenType generic.T, numericTokenType generic.T) *lexer {
	l := &lexer{
		source:            source,
		input:             input,
...
type lexeme struct {
	kind     generic.T    // The type of this lexeme.
...
type lexer struct {
	...
	errorTokenType   generic.T
	numericTokenType generic.T
}
...
func buildLexUntil(findType generic.T, checker checkFn) stateFn {
